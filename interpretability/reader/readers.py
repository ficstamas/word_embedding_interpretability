...
class SeqReader(object):
    ...
    def load_transformer(self, transformer, gpu_id):
        ...
        if 'albert' in transformer:
            tokenizer = AlbertTokenizer.from_pretrained(transformer)
            model = AlbertModel.from_pretrained(transformer, output_hidden_states=True)
        elif 'bert' in transformer:
            tokenizer = BertTokenizer.from_pretrained(transformer)
            model = BertModel.from_pretrained(transformer, output_hidden_states=True)
        elif 'xlnet' in transformer:
            tokenizer = XLNetTokenizer.from_pretrained(transformer)
            model = XLNetModel.from_pretrained(transformer, output_hidden_states=True)
        else:
            raise Exception('Unsupported model type ({})'.format(transformer))
    ...
    def tokenize_sequence(self, sequence):
        ...
        if len(indexed_tokens) == 0:
            logging.warning(("Indexed tokens have 0 length: ", in_file, sequence))
            return None, None
