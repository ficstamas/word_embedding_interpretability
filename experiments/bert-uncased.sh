...
workspace="/data/ficstamas/workspace/bert-uncased/"
proc=30
...
sparse_embedding_name=("bert-base-uncased_layer-12_sparse" "bert-large-uncased_layer-24_sparse" \
                      "bert-base-uncased_layer-9-10-11-12_sparse" "bert-large-uncased_layer-21-22-23-24_sparse")
...
do
    ...
    do
        ...
        python interpretability_cli.py --test_words_weights "$test_embedding" --test_words "$test_words" --embedding_path "$embedding" -dense -numpy --lines_to_read -1 --smc_loader='semcor' --smc_path "$semcor" --processes $proc --distance "$method" --model "contextual" --workspace $workspace --name "${sparse_embedding_name[$i]}_${method}" -save --accuracy "accuracy@1";
    done
done
